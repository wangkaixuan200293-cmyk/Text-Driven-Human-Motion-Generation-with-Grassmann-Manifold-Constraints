...
def attn2img(attn, save_path, title=None):
    """
    saves attention map as an image
    attn: [seq_len1, seq_len2]
    """
    assert attn.shape[0] == 7, "attn shape should be [7, N]"
    img = torch.clamp(attn.detach(), 0, 1).cpu().numpy()
    img = img / img.max()
    # img = (img - img.min()) / (img.max() - img.min())

    # plot with jet cmap but without automatic normalization
    plt.figure(figsize=(attn.shape[1], 7))
    plt.imshow(img, cmap="viridis", aspect="auto", vmin=0, vmax=1)
    plt.colorbar()
    plt.xticks([])
    plt.yticks([])
    plt.grid(False)
    ...
    # display each row with tags at the left of each row
    for i in range(7):
        plt.text(-0.5, i, row_tags[i], ha="right", va="center", fontsize=35)
    

    # title
    if title is not None:
        plt.title(title, pad=20, fontsize=35)
